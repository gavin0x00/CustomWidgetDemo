...
import com.newtrekwang.practice.util.UiUtils;
...
public class LeafProgressBar extends View {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        mLeftMargin= UiUtils.dipToPx(context,LEAF_MARGIN);
...
//    用于控制绘制的进度条距离左/上/下的距离
    private static  final int LEAF_MARGIN=25;
//    用于控制绘制的进度条距离右的距离
...
//    当前所在的绘制的进度条的位置
    private int mCurrentProgressPositon;
//    弧形的半径
...
//        绘制进度条和叶子
    ...
    private void drawProgressAndLeafs(Canvas canvas) {
        ...
        mCurrentProgressPositon=mProgressWidth*mProgress/TOTAL_PROGRESS;
//        当前位置在1范围内
        if (mCurrentProgressPositon<mArcRadius){
            Log.i(TAG, "drawProgressAndLeafs: >>>mProgress: "+mProgress +" mCurrentProgressPositon:  "+mCurrentProgressPositon+ " mArcRadius: "+mArcRadius);
...
//            绘制叶子
            drawLeafs(canvas);
            // 单边角度
            int angle = (int) Math.toDegrees(Math.acos((mArcRadius - mCurrentProgressPositon)
                    / (float) mArcRadius));
            // 起始的位置
            ...
            // 1.绘制white RECT
            mWhiteRectF.left = mCurrentProgressPositon;
            canvas.drawRect(mWhiteRectF, mWhitePaint);
            // 绘制叶子
            drawLeafs(canvas);
            // 2.绘制Orange ARC
            ...
            mOrangeRectF.right = mCurrentProgressPositon;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mWhiteRectF = new RectF(mLeftMargin + mCurrentProgressPositon, mLeftMargin, mTotalWidth
                - mRightMargin,
